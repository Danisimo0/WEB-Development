...
import * as random from '@callumacrae/utils/random.js';
import * as math from '@callumacrae/utils/maths.js';
import getMorseCoder from '@callumacrae/utils/morse-code.js';
import { toVanillaCanvas } from '@callumacrae/utils/renderers/vanilla.js';
import type { Config, InitFn, Framefn, InitProps } from '@callumacrae/utils/renderers/vanilla.d.ts';

const figurePointsEl = document.querySelector('#figure-points');
const figurePoints = JSON.parse(figurePointsEl.innerHTML);
...
async function initSphere(
  ...
  const font = await new Promise<Font>((resolve) => {
    loader.load('https://sketchbook-2.macr.ae/brain-storm/twitter-chirp.json', (font) => resolve(font));
  });
...
export default toVanillaCanvas<CanvasState, SketchConfig>(
  canvasEl,
  init,
  frame,
  sketchbookConfig
